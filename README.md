# Hi there 👋

![GitHub](https://img.shields.io/badge/GitHub-Profile-black?logo=github)
![Profile Views](https://komarev.com/ghpvc/?username=Mukeshthenraj&label=Profile%20views&color=0e75b6&style=flat)
![Followers](https://img.shields.io/github/followers/Mukeshthenraj?label=Followers&style=social)
![Python](https://img.shields.io/badge/Python-3.9-blue?logo=python)
![MATLAB](https://img.shields.io/badge/MATLAB-MathWorks-orange?logo=mathworks)
![Status](https://img.shields.io/badge/status-active-brightgreen)

---

# 👨‍💻 Welcome to My GitHub Profile

### 🚀 About Me

- 🎓 Master's in Automation & Safety – University of Duisburg-Essen  
- 🤖 Focused on Machine Learning, Control Systems, and Data Analysis  
- 🧠 Passionate about AI-powered predictive control and real-time simulations  
- 🌍 Currently seeking opportunities in ML, Control Systems, or Embedded AI

---

### 📂 My Projects

- **[Replay and Visualization of Driving Scenarios in Inland Vessel-Shipping](https://github.com/Mukeshthenraj/Replay-and-Visualization-of-driving-scenarios-in-inland-vessel-shipping)**  
  🚢 C++ and ROS-based project visualizing real-time vessel navigation using simulation data.

- **[Neural Network-Based Structured Model Predictive Control](https://github.com/Mukeshthenraj/Neural-network-based-structured-model-predictive-control-on-a-mass-spring-damper-plane)**  
  🧮 Implementation of LSTM and GRU models in MATLAB for predictive control in dynamic systems.

- **[WhatsApp Chat Analysis](https://github.com/Mukeshthenraj/WhatsApp-chat-analysis)**  
  📊 NLP and visualization project analyzing WhatsApp group behavior using Seaborn and Pandas.

---

### 🛠️ Tech Stack

- **Languages:** Python, C++, ROS2, MATLAB/Simulink, SQL  
- **Frameworks & Libraries:** OpenCV, SciPy, Pandas, NumPy, Seaborn, Matplotlib, TensorFlow, PyTorch, Scikit-learn  
- **Tools:** Git, GitHub, Docker, Kubernetes, Jenkins, VS Code, Power BI, Jupyter Notebook  
- **Specialties:** Control Systems, Predictive Modeling, Real-time Data Processing, Visualization

---

### 📈 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Mukeshthenraj&show_icons=true&theme=default)

---
